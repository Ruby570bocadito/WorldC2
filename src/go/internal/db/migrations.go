...
func (d *DB) Migrate() error {
	...
	for _, m := range migrations {
		...
		// Apply migration inside a transaction so a mid-migration failure
		// cannot leave the schema half-applied (re-runs would then fail on
		// duplicate columns and brick startup).
...
func splitSQL(sql string) []string {
	...
	for i := 0; i < len(sql); i++ {
		...
		// Comments are stripped from the statement stream
		if inLineComment {
			if c == '\n' {
				inLineComment = false
				current.WriteByte(c)
		...
		if !inSingle && !inDouble && c == '-' && i+1 < len(sql) && sql[i+1] == '-' {
			inLineComment = true
			i++
			continue
		}
